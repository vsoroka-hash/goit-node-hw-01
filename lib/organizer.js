...
export class Organizer extends EventEmitter {
  async organize(sourceDir, outputDir) {
    ...
    this.#validatePaths(sourcePath, outputPath);
    this.emit('organize-start', { sourceDir: sourcePath, outputDir: outputPath });

    try {
      await this.#ensureCategoryDirs(outputPath);
    } catch (error) {
      const message = getErrorMessage(error, outputPath);
    ...
    let files;
    try {
      files = await this.#collectFiles(sourcePath, outputPath);
    } catch (error) {
      const message = getErrorMessage(error, sourcePath);
      this.emit('organize-error', { error, message });
      throw new Error(message);
    }
    ...
    for (const filePath of files) {
      try {
        ...
        const destinationDir = path.join(outputPath, category);
  ...
  async #collectFiles(directory, excludedPath) {
    if (this.#isSubPath(excludedPath, directory)) {
      return [];
    }
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        const nested = await this.#collectFiles(fullPath, excludedPath);
        result.push(...nested);
